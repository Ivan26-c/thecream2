...
app.get('/', (req, res) => {
    const htmlFile = fs.existsSync(path.join(__dirname, 'public', 'thecream.html')) ? 'thecream.html' : 'TheCream.html';
...
app.post('/api/admin/banners', upload.single('imagen'), async (req, res) => {
    try {
        const { texto, descripcion, mostrar_boton, activo, imagenUrl } = req.body;
        let imagen = imagenUrl || '';
        if (!imagenUrl && req.file) {
            imagen = 'productos/' + req.file.filename;
        }
...
app.put('/api/admin/banners/:id', upload.single('imagen'), async (req, res) => {
    try {
        const { id } = req.params;
        const { texto, descripcion, mostrar_boton, activo, imagenActual, imagenUrl } = req.body;

        let imagen = imagenUrl || imagenActual;
        if (!imagenUrl && req.file) {
            imagen = 'productos/' + req.file.filename;
        }
